...
Modello Poisson con aggiornamento da Wikipedia (it.wikipedia.org)
Mostra solo la prossima giornata di campionato.
"""
...
def calc_stats(results):
    ...
    for r in results:
        ...
        if home not in stats: stats[home] = {"hgf":0,"hga":0,"hg":0,"agf":0,"aga":0,"ag":0}
        if away not in stats: stats[away] = {"hgf":0,"hga":0,"hg":0,"agf":0,"aga":0,"ag":0}
        stats[home]["hgf"]+=hg; stats[home]["hga"]+=ag; stats[home]["hg"]+=1
        stats[away]["agf"]+=ag; stats[away]["aga"]+=hg; stats[away]["ag"]+=1
    return stats

def league_avg(stats):
    tg = sum(t["hgf"] for t in stats.values()); tm = sum(t["hg"] for t in stats.values())
    return tg/tm if tm>0 else 1.2
...
def predict_serie_b(results, fixtures, range_type="all", custom_n=10):
    filtered = filter_results(results, range_type, custom_n); stats = calc_stats(filtered); avg = league_avg(stats); preds = []
    for fix in fixtures:
        fix = _normalize_fixture(fix); h,a = fix["home"],fix["away"]; hs = stats.get(h); as_ = stats.get(a)
        if not hs or not as_: continue
        hA = hs["hgf"]/hs["hg"] if hs["hg"]>0 else avg; aD = as_["aga"]/as_["ag"] if as_["ag"]>0 else avg
        lam = (hA*aD)/avg; prob = min(1-poisson_pmf(lam,0), 0.99)
        preds.append({"h":h,"a":a,"prob":round(prob,4),"lam":round(lam,2),"hA":round(hA,2),"aD":round(aD,2),"hG":hs["hg"],"aG":as_["ag"],"date":fix.get("date","")})
    preds.sort(key=lambda x: x["prob"], reverse=True)
    return {"predictions":preds, "total":len(filtered), "avg":round(avg,2)}
...
def predict_serie_c(results, fixtures, range_type="all", custom_n=10):
    ...
    for fix in fixtures:
        fix = _normalize_fixture(fix); h,a = fix["home"],fix["away"]; hs = stats.get(h); as_ = stats.get(a)
        if not hs or not as_: continue
        aA = as_["agf"]/as_["ag"] if as_["ag"]>0 else avg*0.8; hD = hs["hga"]/hs["hg"] if hs["hg"]>0 else avg
        lam = (aA*hD)/avg; prob = min(poisson_pmf(lam,0)+poisson_pmf(lam,1), 0.99)
        preds.append({"h":h,"a":a,"prob":round(prob,4),"lam":round(lam,2),"aA":round(aA,2),"hD":round(hD,2),"hG":hs["hg"],"aG":as_["ag"],"date":fix.get("date","")})
    preds.sort(key=lambda x: x["prob"], reverse=True)
    return {"predictions":preds, "total":len(filtered), "avg":round(avg,2)}
...
<body>
<div class="container">
<div class="header"><h1>⚽ Pronostici Serie B & C</h1><p>Modello Poisson — v7 · Dati da Wikipedia · Solo prossima giornata</p></div>
...
function crdB(i,p){const dt=p.date?`<div class="match-date">📅 ${fmtDate(p.date)}</div>`:'';return`<div class="match-card"><div class="rank">#${i+1}</div>${dt}<div class="teams">${p.h} <span class="vs">vs</span> ${p.a}</div><div class="prob-container"><div class="prob-label"><span class="type">CASA OVER 0.5</span><span class="pct" style="color:${pcol(p.prob)}">${(p.prob*100).toFixed(1)}%</span></div><div class="prob-bar"><div class="prob-fill ${pc(p.prob)}" style="width:${p.prob*100}%"></div></div></div><div class="detail-stats"><div class="detail-stat"><div class="dl">λ Casa</div><div class="dv">${p.lam}</div></div><div class="detail-stat"><div class="dl">Att / Def</div><div class="dv">${p.hA} / ${p.aD}</div></div><div class="detail-stat"><div class="dl">P. Casa</div><div class="dv">${p.hG}</div></div><div class="detail-stat"><div class="dl">P. Trasf.</div><div class="dv">${p.aG}</div></div></div></div>`}
function crdC(i,p){const dt=p.date?`<div class="match-date">📅 ${fmtDate(p.date)}</div>`:'';return`<div class="match-card"><div class="rank">#${i+1}</div>${dt}<div class="teams">${p.h} <span class="vs">vs</span> ${p.a}</div><div class="prob-container"><div class="prob-label"><span class="type">OSPITE UNDER 1.5</span><span class="pct" style="color:${pcol(p.prob)}">${(p.prob*100).toFixed(1)}%</span></div><div class="prob-bar"><div class="prob-fill ${pc(p.prob)}" style="width:${p.prob*100}%"></div></div></div><div class="detail-stats"><div class="detail-stat"><div class="dl">λ Ospite</div><div class="dv">${p.lam}</div></div><div class="detail-stat"><div class="dl">Att / Def</div><div class="dv">${p.aA} / ${p.hD}</div></div><div class="detail-stat"><div class="dl">P. Casa</div><div class="dv">${p.hG}</div></div><div class="detail-stat"><div class="dl">P. Trasf.</div><div class="dv">${p.aG}</div></div></div></div>`}
