...
Modello Poisson con aggiornamento da calciomagazine.net (tabelle risultati)
...
def calc_stats(results):
    """Calculate team stats from results list (handles both v6 and v7 formats)."""
    stats = {}
    for r in results:
...
def predict_serie_b(results, fixtures, range_type="all", custom_n=10):
    """Serie B: Casa Over 0.5 — predictions only for next giornata fixtures."""
...
def predict_serie_c(results, fixtures, range_type="all", custom_n=10):
    """Serie C: Ospite Under 1.5 — predictions only for next giornata fixtures."""
    filtered = filter_results(results, range_type, custom_n)
    stats = calc_stats(filtered)
    avg = league_avg(stats)
    predictions = []
...
<p>Modello Poisson — v7 · Dati da calciomagazine.net · Solo prossima giornata</p>
</div>
